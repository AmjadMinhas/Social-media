<script setup>
    ...
    const props = defineProps(['contactGroups','contact', 'displayContactInfo', 'ticketingIsEnabled', 'ticket', 'addon']);
    ...
    const isOpenGroupModal = ref(false);
    const user = ref({ 
    ...
    const form4 = useForm({
        ...
        group: props.contact?.contact_groups
            ? props.contact.contact_groups.map(group => group.uuid || group)
            : [],
        ...
        metadata: props.contact?.metadata || null,
        file: null
    });

    const submitForm4 = () => {
        console.log(form4.data());
        form4.post('/contacts/' + props.contact.uuid, {
            preserveState: false,
            onSuccess: () => {
                form4.reset();
                isOpenGroupModal.value = false;
            }
        });
    }
</script>
<template>
                ...
                <Dropdown v-if="!displayContact">
                    ...
                    <template #items>
                        <DropdownItemGroup>
                            ...
                            <DropdownItem @click="isOpenGroupModal = true;" as="button">{{ $t('Add Group') }}</DropdownItem>
    ...
    <Modal :label="$t('Edit Group')" :isOpen="isOpenGroupModal">
        <div class="grid grid-cols-1 gap-x-6 gap-y-4">
            <form @submit.prevent="submitForm4()" class="">
            <div class="grid grid-cols-1 gap-x-6 gap-y-4 overflow-y-auto"  style="height: 120px;">
                <FormSelect
                    v-model="form4.group"
                    :name="$t('Group')"
                    :error="form4.errors.group"
             ...
             <div class="mt-4 flex">
                    <button type="button" @click.self="isOpenGroupModal = false" class="inline-flex justify-center rounded-md border border-transparent bg-slate-50 px-4 py-2 text-sm text-slate-500 hover:bg-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 mr-4">{{ $t('Cancel') }}</button>
                    <button 
                        :class="['inline-flex justify-center rounded-md border border-transparent bg-primary px-4 py-2 text-sm text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2', { 'opacity-50': form4.processing }]"
                        :disabled="form4.processing"
                    >
                        <svg v-if="form4.processing" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2A10 10 0 1 0 22 12A10 10 0 0 0 12 2Zm0 18a8 8 0 1 1 8-8A8 8 0 0 1 12 20Z" opacity=".5"/><path fill="currentColor" d="M20 12h2A10 10 0 0 0 12 2V4A8 8 0 0 1 20 12Z"><animateTransform attributeName="transform" dur="1s" from="0 12 12" repeatCount="indefinite" to="360 12 12" type="rotate"/></path></svg>
                        <span v-else>{{ $t('Save') }}</span>
                    </button>
                </div>
            </form>
