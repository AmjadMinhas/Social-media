...
class DashboardController extends BaseController
{
    ...
    public function index(Request $request){
        //check if the user has an active organization deleted or not
        $user = auth()->user();
        $currentOrgId = session('current_organization');
        if (!$currentOrgId || !Organization::where('id', $currentOrgId)->exists()) {
            if ($user->organizations()->count() > 1) {
                return redirect()->route('user.organization.index');
            }
            $org = $user->organizations()->first();
            if ($org) {
                session()->put('current_organization', $org->id);
                $currentOrgId = $org->id;
            } else {
                return redirect()->route('user.organization.index');
            }
