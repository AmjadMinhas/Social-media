...
function addVariable(variable: string) {
  const wrapper = textAreaRef.value?.$el as HTMLElement
...
function removeRow(sectionIndex, rowIndex) {
  if (fields.value.sections[sectionIndex].rows.length > 1) {
    ...
    fields.value.sections[sectionIndex].rows = fields.value.sections[sectionIndex].rows.map((row) => ({
      ...row,
      id: row.id,
      title: row.title || "",
...
const shouldShowWarning = computed(() => {
  const hasEmptySections = fields.value.sections.some(section => {
    ...
    return section.rows.every(row => !row.title || !row.id)
  });
  return (
...
function handleInput(e: Event) {
  ...
  fields.value.buttonLabel = removeEmojis(input).slice(0, 20)
}
  ...
  <div class="rounded-sm border border-gray-200 bg-white p-3 shadow-md">
    ...
    <div class="flex flex-col gap-y-2">
      ...
      <div v-if="fields.headerType == 'text'" class="mb-4">
        ...
        <FormInput v-model="fields.headerText" :name="''" :placeholder="'Enter header text'" :type="'text'" :class="'col-span-4'"  :maxlength="60"/>
      ...
      <div class="mb-4">
        <label class="text-sm mb-2 flex justify-between items-center">
          <span><span class="text-red-500">*</span> Body</span>
        </label>
        <FormTextArea
          ref="textAreaRef"
          v-model="fields.body"
          :placeholder="'Enter the main message for this message type'"
          :name="''"
          :type="'text'"
          :class="'col-span-4'"
        />
      ...
      <div class="mb-4">
        ...
        <FormInput v-model="fields.buttonLabel" :name="''" :placeholder="'Enter footer text'" :type="'text'" :class="'col-span-4'" :maxlength="20"
          @input="(e) => handleInput(e)"
          @paste="(e) => handlePaste(e)"/>
        <span class="text-xs text-gray-500">{{ getCharsCount(fields.buttonLabel) }}/20</span>
      </div>
      ...
      <div v-for="(section, sectionIndex) in fields.sections" :key="sectionIndex" class="border rounded p-3 mb-4">
        ...
        <div class="mb-4">
          ...
          <FormInput v-model="section.title" :name="''" :placeholder="'Enter section title'" :type="'text'" :class="'col-span-4'" :maxlength="24"
            @input="(e) => handleInput(e)"
            @paste="(e) => handlePaste(e)"/>
          <span class="text-xs text-gray-500">{{ getCharsCount(section.title ?? '') }}/24</span>
        </div>
